...
import { getCharSpriteKey, CHAR_STATIC_SPRITES, CHAR_SCALE_VS, DIFFICULTY_COLORS } from '../utils/Constants.js';
...
export default class VSIntroScene extends Phaser.Scene {
    ...
    _showPreMatchDialogue(onDone) {
        ...
        overlay.fillRect(0, Layout.H * 0.6, Layout.W, Layout.H * 0.4);

        // Skip hint
        const skipHint = this.add.text(Layout.W - 16, Layout.H - 14, I18n.t('vsintro.skip_hint'), {
        ...
        const showLine = (index) => {
            ...
            const dialogText = this.add.text(28, boxY + 14, '', {
                ...textStyle, wordWrap: { width: Layout.W - 56 }
            });
    ...
    _runVSAnimation() {
        ...
        bg.beginPath();
        bg.moveTo(Layout.W * 0.35, 0);
        bg.lineTo(Layout.W, 0);
        bg.lineTo(Layout.W, Layout.H);
        bg.lineTo(Layout.W * 0.65, Layout.H);
        bg.closePath();
        bg.fillPath();
        ...
        divider.beginPath();
        divider.moveTo(Layout.W * 0.35, 0);
        divider.lineTo(Layout.W * 0.65, Layout.H);
        divider.strokePath();

        // Round number (if arcade)
        if (this.roundNumber) {
            this.add.text(Layout.W / 2, 20, `ROUND ${this.roundNumber}`, {
                fontFamily: 'monospace', fontSize: '18px', color: '#D4A574', shadow: SHADOW
            }).setOrigin(0.5).setAlpha(0);
        }

        // VS text (center, big)
        const vsText = this.add.text(Layout.W / 2, Layout.H / 2, 'VS', {
        ...
        // Player side (left)
        const playerX = Layout.W * 0.22;
        const playerNameText = this.add.text(playerX, Layout.H / 2 - 60, I18n.field(player, 'name').toUpperCase(), {
            fontFamily: 'monospace', fontSize: '28px', color: '#87CEEB', shadow: SHADOW
        }).setOrigin(0.5).setX(-200);

        const playerTitleText = this.add.text(playerX, Layout.H / 2 - 30, I18n.field(player, 'title'), {
        ...
        if (this.textures.exists(playerSpriteKey)) {
            if (playerIsStatic) {
                playerSprite = this.add.image(playerX, Layout.H / 2 + 30, playerSpriteKey)
                    .setScale(CHAR_SCALE_VS).setOrigin(0.5).setX(-200);
            } else {
                playerSprite = this.add.sprite(playerX, Layout.H / 2 + 40, playerSpriteKey, 0)
                    .setScale(CHAR_SCALE_VS).setOrigin(0.5).setX(-200);
            }
        }

        // Opponent side (right)
        const opponentX = Layout.W * 0.78;
        const opponentNameText = this.add.text(opponentX, Layout.H / 2 - 60, I18n.field(opponent, 'name').toUpperCase(), {
            fontFamily: 'monospace', fontSize: '28px', color: '#C44B3F', shadow: SHADOW
        }).setOrigin(0.5).setX(Layout.W + 200);

        const opponentTitleText = this.add.text(opponentX, Layout.H / 2 - 30, I18n.field(opponent, 'title'), {
        ...
        if (this.textures.exists(opponentSpriteKey)) {
            if (opponentIsStatic) {
                opponentSprite = this.add.image(opponentX, Layout.H / 2 + 30, opponentSpriteKey)
                    .setScale(CHAR_SCALE_VS).setOrigin(0.5).setFlipX(true).setX(Layout.W + 200);
            } else {
                opponentSprite = this.add.sprite(opponentX, Layout.H / 2 + 40, opponentSpriteKey, 0)
                    .setScale(CHAR_SCALE_VS).setOrigin(0.5).setFlipX(true).setX(Layout.W + 200);
            }
        }
        ...
        if (opponentBarkArr) {
            ...
            const barkText = this.add.text(opponentX, Layout.H / 2 + 90, `"${bark}"`, {
                fontFamily: 'monospace', fontSize: '11px', color: '#F5E6D0',
                shadow: SHADOW, wordWrap: { width: 200 }, align: 'center',
                fontStyle: 'italic'
            }).setOrigin(0.5).setX(Layout.W + 200).setAlpha(0.9);
        ...
        if (opponent.catchphrase) {
            opponentCatchphraseText = this.add.text(opponentX, Layout.H / 2 - 8,
            ...
            ).setOrigin(0.5).setX(Layout.W + 200).setAlpha(0);
        }

        // Terrain name at bottom
        const terrainText = this.add.text(Layout.W / 2, Layout.H - 65, this.terrainName, {
        ...
        const diffLabel = this.add.text(Layout.W / 2, Layout.H - 45, diffMap[diff] || diff.toUpperCase(), {
        ...
        let hintText = null;
        if (this.introText) {
            hintText = this.add.text(Layout.W / 2, Layout.H - 35, this.introText, {
        ...
        this.tweens.add({
            ...
            onComplete: () => {
                ...
                if (catchphrase) {
                    const catchphraseText = this.add.text(playerX, Layout.H / 2 - 8,
                        `"${catchphrase}"`, {
                            fontFamily: 'monospace', fontSize: '12px', color: '#D4A574',
                            shadow: SHADOW, fontStyle: 'italic',
        ...
        // 4. "MATCH !" text slam
        const matchText = this.add.text(Layout.W / 2, Layout.H / 2 + 50, 'MATCH !', {
